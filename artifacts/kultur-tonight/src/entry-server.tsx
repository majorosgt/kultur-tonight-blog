/**
 * SSR entry point used exclusively by the prerender build step.
 * This module is never loaded by the browser.
 */
import { renderToString } from "react-dom/server";
import { Switch, Route, Router as WouterRouter, Redirect } from "wouter";
import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
import { SSRSEOContext, type SEOConfig, type SSRSEOContextValue } from "@/lib/seo";

/**
 * A minimal static location hook for server-side rendering.
 * Does NOT use useSyncExternalStore, so it works correctly with
 * React 19's renderToString (which requires getServerSnapshot when
 * useSyncExternalStore is used, something wouter/memory-location omits).
 */
function makeStaticHook(path: string): () => [string, () => void] {
  return function useStaticLocation() {
    return [path, () => {}];
  };
}

// Static imports — React.lazy is incompatible with renderToString
...
// ── Prerender slug manifest ───────────────────────────────────────────────────
// Re-exported so prerender.mjs can discover dynamic routes by importing them
// from the single SSR bundle (dist/server/entry-server.js).  Do NOT import
// these at runtime in the browser — entry-server.tsx is never shipped to clients.
export { events } from "@/content/events";
export { eventsFr } from "@/content/events.fr";
export { venues } from "@/content/venues";
export { venuesFr } from "@/content/venues.fr";
export { blogGuides } from "@/content/blog-guides";
export { blogGuidesFr } from "@/content/blog-guides.fr";
export { blogVenues } from "@/content/blog-venues";
export { blogVenuesFr } from "@/content/blog-venues.fr";
export { blogThisWeek } from "@/content/blog-this-week";
export { blogThisWeekFr } from "@/content/blog-this-week.fr";
export { blogThisWeekend } from "@/content/blog-this-weekend";
export { blogThisWeekendFr } from "@/content/blog-this-weekend.fr";
export { blogEvents } from "@/content/blog-events";
export { blogEventsFr } from "@/content/blog-events.fr";
export { blogSeasonal } from "@/content/blog-seasonal";
export { blogSeasonalFr } from "@/content/blog-seasonal.fr";
export { blogCulture } from "@/content/blog-culture";
export { blogCultureFr } from "@/content/blog-culture.fr";
export { blogFamily } from "@/content/blog-family";
export { blogFamilyFr } from "@/content/blog-family.fr";
export { renderSEOHead } from "@/lib/seo";

export function render(url: string): { html: string; seo: SEOConfig | null } {
  const seoState: SSRSEOContextValue = {
    seo: null,
    capture(s: SEOConfig) {
      if (this.seo === null) this.seo = s;
    },
  };
  ...
  const html = renderToString(
    <SSRSEOContext.Provider value={seoState}>
      <QueryClientProvider client={new QueryClient()}>
        <WouterRouter hook={hook}>
          <SSRRouter />
        </WouterRouter>
      </QueryClientProvider>
    </SSRSEOContext.Provider>
  );

  return { html, seo: seoState.seo };
}
